...
\begin{tikzpicture}
   ...
   \draw [blue!50,ultra thick,domain=50:55] plot ({1.6*cos(\x)}, {1.5*sin(\x)});
   \draw [red,ultra thick,domain=56:100] plot ({1.6*cos(\x)}, {1.5*sin(\x)});
   \draw [blue, ultra thick, domain=101:160] plot ({1.6*cos(\x)}, {1.5*sin(\x)});
   \draw [blue!50, ultra thick, domain=101:106] plot ({1.6*cos(\x)}, {1.5*sin(\x)});
...
\end{itemize}
This methodology is informative for the sequence integrity, it does not however show the unitig that boards the misassembly or the continuity rupture. This is done with the graphical output of \texttt{graph\_constructor.py}. Unscaffolded unitigs and wrongly scaffolded unitigs are detected. Their environment in the input data is analyzed in order to understand the reason behind the scaffolding failure.
...
\caption{GST input graph features}
...
\# scaffolds ($\geq$ 0 bp) & 6 & {\bf 1} & {\bf 1} & {\bf 1} & {\bf 1} & 2 & 3 & {\bf 1} \\ \hline
...
\newpage
\subsubsection{Quality assessment of genome assemblies of \textit{Pinus koraiensis}}
